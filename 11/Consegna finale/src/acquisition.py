...
class BiometricAcquisition:
    ...
    def __init__(self,
        ...
        """
        Args:
            symmetry_threshold: 0.0=Perfetto, >0.20 inizia a essere girato orizzontalmente.
            pitch_range: (min, max). Rapporto (Occhi-Naso)/(Naso-Bocca).
                         Sotto min = Guarda in ALTO. Sopra max = Guarda in BASSO.
        """
    ...
    def _compute_metrics(self, img):
        """Calcola metriche di base dell'immagine"""
    ...
    def _check_head_pose(self, landmarks):
        """
        Stima la posa della testa (Yaw e Pitch) dai 5 punti facciali.
        Landmarks FaceNet: 0=OcchioSx, 1=OcchioDx, 2=Naso, 3=BoccaSx, 4=BoccaDx
        """
        eye_l = landmarks[0]
        eye_r = landmarks[1]
        nose = landmarks[2]
        ...
        if dist_l_x == 0 or dist_r_x == 0: return False, 1.0, False, 0.0  # Errore calcolo
    ...
    def _extract_and_score_face(self, img):
        if img is None: return None, {}, "Immagine non valida."
        ...
        if boxes is None or len(boxes) == 0:
            return None, {}, "Nessun volto."
        if len(boxes) > 1:
            return None, {}, f"ERRORE: {len(boxes)} volti rilevati."
    ...
    def _is_quality_pass(self, metrics):
        if not metrics['is_yaw_ok']:
            print(f"  -> RIFIUTATO: Testa girata orizzontalmente ({metrics['yaw_score']:.2f})")
            return False

        if not metrics['is_pitch_ok']:
            print(
                f"  -> RIFIUTATO: Testa girata verticalmente (Guarda in {metrics['pitch_str']}, ratio {metrics['pitch_ratio']:.2f})")
            return False

        # 3. Controllo Qualità Immagine
    ...
    def acquire_best_sample(self, capture_function, filename='face.jpg'):
        ...
        for i in range(1, self.max_attempts + 1):
            ...
            if face is not None:
                ...
                if metrics['is_yaw_ok'] and metrics['is_pitch_ok']:
                ...
                if self._is_quality_pass(metrics):
                    print("Qualità e Posa 3D accettate!")
        ...
        if best_face is not None:
            print("ATTENZIONE: Nessun campione perfetto. Uso il migliore disponibile.")
            cv2.imwrite(filename, best_face)
            return best_face, time.time()
